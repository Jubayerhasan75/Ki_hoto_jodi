...
const TimeCalculator: React.FC = () => {
  ...
  const skillsLearned = (yearlySavedHours / 100).toFixed(1); // 100 hours per skill
  ...
  const fetchInsight = useCallback(async (hours: number) => {
    if (hours <= 0) {
      setInsight("সময় বাঁচাতে শুরু করুন!");
      return;
    }
    setLoadingInsight(true);
    const result = await getSavingsInsight(hours, 'TIME');
    setInsight(result);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      fetchInsight(yearlySavedHours);
    }, 1500);
    return () => clearTimeout(timer);
  }, [yearlySavedHours, fetchInsight]);
